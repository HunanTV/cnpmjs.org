...
/**
 * 列出所有包
 * Author: ec.huyinghuan@gmail.com Date: 2017.4.10
 */
exports.listAll = function *(pageIndex, limit){
  pageIndex = pageIndex || 1
  limit = limit || 40
  var sql = `SELECT M.*, T.version as latest_version FROM module M 
	            LEFT JOIN tag T ON M.id = T.module_id
              LIMIT ?,?;`
  return yield* models.query(sql, [(pageIndex - 1) * limit, limit]);
}

/**
 * 列出某个作者所有包
 * Author: ec.huyinghuan@gmail.com Date: 2017.4.11
 */
exports.listAuthorPackage = function*(author, pageIndex, limit){
  pageIndex = pageIndex || 1
  limit = limit || 40
  var sql = `SELECT M.*, T.version as latest_version
              FROM module M 
	              LEFT JOIN tag T 
                  ON M.id = T.module_id
                WHERE M.author = ? LIMIT ?,?;`
  return yield [models.query(sql, [author, (pageIndex - 1) * limit, limit]), models.query( 'SELECT count(*) as totalPage FROM module WHERE author = ?;', [author])]
}
